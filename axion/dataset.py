...
class Dataset(RichSerializer):
    ...
    def add_item(
        ...
        if isinstance(item, DatasetItem):
            ...
            if ignore_extra_keys:
                # Filter dict to only include valid DatasetItem fields
                # Get all model field keys
                valid_fields = set(DatasetItem.model_fields.keys())
    ...
    def add_items(
        ...
        Args:
            ...
            ignore_extra_keys: If True, only use keys that match DatasetItem fields,
                ignoring any extra keys in dictionaries. Defaults to False.
        ...
        added_items = []
        for item in items:
            added_items.append(self.add_item(item, ignore_extra_keys=ignore_extra_keys))
        return added_items
    ...
    def read_dataframe(
        ...
        **kwargs,
        ...
        for record in records:
            ...
            try:
                dataset.add_item(record, ignore_extra_keys=ignore_extra_keys)
            except CustomValidationError as e:
                item_id = record.get(FieldNames.ID, 'N/A')
                logger.error(
                    f"Failed to validate item '{item_id}' from DataFrame — possible schema mismatch or invalid data. Error: {e}"
                )
    ...
    def read_json(
        ...
        for item_dict in items_data:
            ...
            if FieldNames.QUERY in item_dict or FieldNames.CONVERSATION in item_dict:
                try:
                    dataset.add_item(item_dict, ignore_extra_keys=ignore_extra_keys)
                except CustomValidationError as e:
    ...
    def read_csv(
        ...
        **kwargs,
        ...
        return cls.read_dataframe(
            dataframe, name=name, ignore_extra_keys=ignore_extra_keys, **kwargs
        )
