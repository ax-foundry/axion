...
class HitRateAtK(_RetrievalMetric):
    ...
    @trace(name='HitRateAtK', capture_args=True)
...
class MeanReciprocalRank(_RetrievalMetric):
    ...
    @trace(name='MeanReciprocalRank', capture_args=True)
    async def execute(self, item: DatasetItem, **kwargs) -> MetricEvaluationResult:
        actual_ids, expected_relevance_map, error = self._parse_item(item)
        if error:
            return error
...
class NDCGAtK(_RetrievalMetric):
    ...
    @trace(name='NDCGAtK', capture_args=True)
    async def execute(self, item: DatasetItem, **kwargs) -> MetricEvaluationResult:
        actual_ids, expected_relevance_map, error = self._parse_item(item)
        if error:
            return error
...
class PrecisionAtK(_RetrievalMetric):
    ...
    @trace(name='PrecisionAtK', capture_args=True)
    async def execute(self, item: DatasetItem, **kwargs) -> MetricEvaluationResult:
        actual_ids, expected_relevance_map, error = self._parse_item(item)
        if error:
            return error
...
class RecallAtK(_RetrievalMetric):
    ...
    @trace(name='RecallAtK', capture_args=True)
    async def execute(self, item: DatasetItem, **kwargs) -> MetricEvaluationResult:
        actual_ids, expected_relevance_map, error = self._parse_item(item)
        if error:
            return error
